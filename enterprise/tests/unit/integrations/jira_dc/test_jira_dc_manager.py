...
class TestIsJobRequested:
    ...
    async def test_is_job_requested_new_conversation_with_repo_match(
        ...
        jira_dc_manager._get_provider_handler = AsyncMock(return_value=mock_provider_handler)

        with patch(
            'integrations.jira_dc.jira_dc_manager.verify_inferred_repository'
        ) as mock_verify:
    ...
    async def test_is_job_requested_no_provider_tokens(
        ...
        jira_dc_manager._get_provider_handler = AsyncMock(return_value=None)
        jira_dc_manager._send_repo_selection_comment = AsyncMock()
        ...
        assert result is False
        jira_dc_manager._send_repo_selection_comment.assert_called_once_with(mock_view)
    ...
    async def test_is_job_requested_exception(self, jira_dc_manager, sample_user_auth):
        ...
        mock_view = MagicMock(spec=JiraDcNewConversationView)
        mock_view.saas_user_auth = sample_user_auth
        jira_dc_manager._get_provider_handler = AsyncMock(

...
class LinearManager(Manager):
    ...
    async def validate_request(
        ...
        if hmac.compare_digest(signature, digest):
            logger.info(
                '[Linear] Webhook signature verified successfully',
    ...
    async def is_job_requested(
        ...
        try:
            ...
            if not provider_handler:
                logger.warning(
                    '[Linear] Job request failed: no provider tokens available',
                    extra={
            ...
            if result.success:
                ...
                linear_view.selected_repo = result.repository.full_name
                logger.info(f'[Linear] Verified repository: {result.repository.full_name}')
                return True
            else:
                # Log the specific failure reason
                logger.warning(
                    '[Linear] Job request failed: repository verification unsuccessful',
                    extra={
            ...
            logger.error(
                f'[Linear] Error in is_job_requested: {str(e)}',
